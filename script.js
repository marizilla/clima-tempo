...
//Acessando os dados salvos  
  if(localStorage.getItem("Dados Salvos")){
    ...
    var msg = "";
    for (i=0; i< historico.length; i++){msg += '<div class="historico"> <div class="dados"> <p class="data">' + historico[i].day + " " + months[historico[i].month] + " " + historico[i].year + ' | ' + historico[i].hour + ":" + historico[i].min + '</p> <p class="temp">' + Math.round(historico[i].main.temp) + "&deg; C" + 
              ' <br> '
              + historico[i].weather[0].description + '</p> </div> <div class="imagem"> <img src="http://openweathermap.org/img/wn/' + historico[i].weather[0].icon + '@2x.png"> </div> </div>';
...
if ("geolocation" in navigator) {
  navigator.geolocation.getCurrentPosition(function(position){
      ...
      const getWeather = async () => {
          try{
              const resp = await fetch(apiWork);
